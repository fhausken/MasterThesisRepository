...
tryCatch({
  file.remove(paste(URL.repo,"/Data/data.bin",sep=""))
  file.remove(paste(URL.repo,"/Data/data.desc",sep=""))
  },error=function(e) e)
    ...
    individualStockResults=foreach(day=0:rollingWindowSize) %dopar%{ 
      ...
      for (distributions.index in 1:length(distributions)){
        ...
        fit.distribution=tryCatch({
          ...
          setTimeLimit() #Clears time limit
        ...
        if(is(fit.distribution,"warning")){
          ...
          warningMessage=fit.distribution[[1]]
          
          if (warningMessage=="reached CPU time limit"){
            cat(paste(Sys.time(), "\t\t\t","Iteration: ",stocksIndex,"/" , nrow(stocks),". Stock: ",stocks[stocksIndex,1] ,". Sample size: ",sampleSize,". Day: ",day,"/" , rollingWindowSize,". Timeout i distribution fit!","\n",sep=""), file=URL.logging, append=TRUE)
          } else{
            ...
            for (GARCHLagOne in 1:GARCHLagOne.max){
              for (GARCHLagTwo in 1:GARCHLagTwo.max){
                ...
                if(is(fit,"warning")){
                  ...
                  warningMessage=fit[[1]]
                  
                  if (warningMessage=="reached CPU time limit"){
                    cat(paste(Sys.time(), "\t\t\t\t","Iteration: ",stocksIndex,"/" , nrow(stocks),". Stock: ",stocks[stocksIndex,1] ,". Sample size: ",sampleSize,". Day: ",day,"/" , rollingWindowSize,". Distribution: ",bestDistributionFit.fullname, ". Model: ",garchModel,"(",ARLag,MALag,GARCHLagOne,GARCHLagTwo,"). Timeout i ARMA GARCH fit!","\n",sep=""), file=URL.logging, append=TRUE)
                  }else{
      ...
      cat("Output:\n\n", file=URL.progress, append=FALSE) 
      cat(paste(Sys.time(), "\n\n"), file=URL.progress, append=TRUE) 
      cat(daysLeftAsVector, file=URL.progress, append=TRUE) 
      
      # }, error=function(e) {
      #   cat(paste(Sys.time(), "\t\t\t","Iteration: ",stocksIndex,"/" , nrow(stocks),". Stock: ",stocks[stocksIndex,1] ,". Sample size: ",sampleSize,". Day: ",day,"/" , rollingWindowSize,". Distribution: ",bestDistributionFit.fullname, ". Model: " ,garchModel.final,"(",ARLag.final,MALag.final,GARCHLagOne.final,GARCHLagTwo.final,"). Iterasjon fullført, men ikke fjernet dag fra daysLeft!","\n",sep=""), file=URL.logging, append=TRUE) 
      # })
